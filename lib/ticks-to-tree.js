...
function ticksToTree (ticks, args, mapFrames, inlined) {
  const merged = {
    name: 'all stacks',
    value: 0,
    top: 0,
        ...
        if (inlineInfo === true) {
          ...
          if (info !== undefined) {
            ...
            if (sameFn === false) {
              ...
              const lookupRaw = stack[ix - 1] && stack[ix - 1].name.split(':')
              if (lookupRaw && lookupRaw.length !== 0) {
                const lookup = lookupRaw[0]
                for (const { caller } of inlined[key]) {
                  if (caller.key === lookup) {
                    S += 2
                    break
                  }
                }
  ...
  function addToUnmergedTree (stack) {
    ...
    stack.forEach((frame, ix) => {
      ...
      for (const child of children) {
        if (child.fn === frame.name && child.S === frame.S) {
          match = child
          break
        }
      }
